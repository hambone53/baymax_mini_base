...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode)
{
  ...
  */
  ...
  delay(10);
	...
	/* Hamor set the default units */
	_units.accUnit = ACC_UNIT_MS2;
	_units.gyrUnit = GYR_UNIT_DPS;
	_units.eulUnit = EUL_UNIT_DEG;
	_units.tempUnit = TEMP_UNIT_C;
	_units.oriUnit = ORI_UNIT_WIN;
...
imu::Vector<3> Adafruit_BNO055::getVector(adafruit_vector_type_t vector_type)
{
  ...
  //Hamor modified to match address information for Euler information it is heading (z), Roll (y), and pitch (x) this will match the default coordinate system.
  switch(vector_type)
  {
		case VECTOR_EULER:    
  		z = ((int16_t)buffer[0]) | (((int16_t)buffer[1]) << 8);
  		y = ((int16_t)buffer[2]) | (((int16_t)buffer[3]) << 8);
  		x = ((int16_t)buffer[4]) | (((int16_t)buffer[5]) << 8);
			break;
		default:
  ...
  switch(vector_type)
  {
    ...
    case VECTOR_GYROSCOPE:
      /* 1rps = 900 LSB */
			if( _units.gyrUnit == GYR_UNIT_DPS )
			{
      	xyz[0] = ((double)x)/16.0;
      	xyz[1] = ((double)y)/16.0;
      	xyz[2] = ((double)z)/16.0;
    ...
    case VECTOR_EULER:
      /* 1 degree = 16 LSB */
			if ( _units.eulUnit == EUL_UNIT_DEG )
			{
      	xyz[0] = ((double)x)/16.0;
      	xyz[1] = ((double)y)/16.0;
      	xyz[2] = ((double)z)/16.0;
    ...
    case VECTOR_GRAVITY:
			if ( _units.accUnit == ACC_UNIT_MS2 )
			{
      	/* 1m/s^2 = 100 LSB */
...
/**************************************************************************/
/*!
    @brief  Sets the units of the different output modes
*/
/**************************************************************************/
void Adafruit_BNO055::setUnits( bno055_units_type_t units )
{
	adafruit_bno055_opmode_t modeback = _mode;
  ...
  delay(25);
	...
	uint8_t unitSelect = (units.oriUnit << 7) |
											 (units.tempUnit << 4) |
											 (units.eulUnit << 2) |
											 (units.gyrUnit << 1) |
											 (units.accUnit << 0);
	
	//unitSelect = ( 0x10 & (units.tempUnit << 4) ) + unitSelect;
	//unitSelect = ( 0x04 & (units.eulUnit << 2) ) + unitSelect;
	//unitSelect = ( 0x02 & (units.gyrUnit << 1) ) + unitSelect;
	//unitSelect = ( 0x01 & units.accUnit) + unitSelect;

	_units = units;

  write8(BNO055_UNIT_SEL_ADDR, unitSelect);
  delay(30);
...
/**************************************************************************/
/*!
    @brief  gets the units of the different output modes
*/
/**************************************************************************/
Adafruit_BNO055::bno055_units_type_t Adafruit_BNO055::getUnits(void)
{
	bno055_units_type_t unitsToReturn;
